...
@section('content')
    ...
    <div class="item-detail-sm-preview d-lg-none">
        ...
        @endif
        @if (!empty($hasPurchased))
            <a href="{{ route('product.download.paid', $product->slug) }}" class="btn btn--base w-100">
                <span class="icon"><i class="fas fa-download"></i></span>
                Download Now
            </a>
        @elseif ($pRegular <= 0)
...
@push('style')
<style>
    .install-float-ad{position:fixed;left:20px;bottom:20px;z-index:9990;max-width:320px;background:hsl(var(--primary));color:#fff;border-radius:12px;box-shadow:0 12px 32px hsl(var(--primary) / .35);padding:14px 16px;display:flex;gap:12px;align-items:flex-start;animation:ifaSlideUp .5s ease both}
    .install-float-ad.is-hidden{display:none}
    .install-float-ad .ifa-icon{width:44px;height:44px;flex:0 0 44px;background:#fff;color:hsl(var(--primary));border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px}
    .install-float-ad .ifa-body{flex:1;min-width:0}
    .install-float-ad .ifa-title{font-weight:700;font-size:15px;line-height:1.25;margin:0 0 4px;color:#fff}
    .install-float-ad .ifa-text{font-size:13px;line-height:1.45;margin:0 0 10px;color:rgba(255,255,255,.92)}
    .install-float-ad .ifa-cta{display:inline-flex;align-items:center;gap:6px;background:#fff;color:hsl(var(--primary));font-weight:600;font-size:13px;padding:6px 14px;border-radius:6px;border:0;text-decoration:none}
    .install-float-ad .ifa-cta:hover{background:hsl(var(--primary-l-500));color:hsl(var(--primary))}
    .install-float-ad .ifa-close{position:absolute;top:6px;right:10px;background:transparent;border:0;color:#fff;font-size:18px;line-height:1;cursor:pointer;opacity:.85}
