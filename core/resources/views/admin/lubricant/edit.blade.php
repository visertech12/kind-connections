...
<div class="page-header">
    <div>
        <div class="page-header__title">Edit Lubricant</div>
        <div class="page-header__subtitle">Update the details of this lubricant listing</div>
    </div>
...
<div style="display:grid;grid-template-columns:1fr 320px;gap:20px;align-items:start">
    ...
    <div>
        <div class="card" style="margin-bottom:20px">
            ...
            <form action="{{ route('admin.lubricant.update', $lubricant->id) }}" method="POST" enctype="multipart/form-data" id="lubricantForm">
                @csrf
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" class="form-control" required>{{ old('description', $lubricant->description) }}</textarea>
                    </div>

                    <div class="form-row">
                        <div class="form-group">
                            <label class="form-label">Regular Price ($) <span class="required">*</span></label>
                            <input type="number" step="0.01" min="0" name="regular_price" class="form-control" value="{{ old('regular_price', (float)$lubricant->regular_price) }}" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <div style="border:2px dashed var(--border-color);border-radius:var(--radius-md);padding:28px;text-align:center;cursor:pointer;transition:var(--transition);position:relative"
                            ...
                            <div id="dropPlaceholder" style="{{ $lubricant->image ? 'display:none' : '' }}">
                            ...
                            @php
                                $thumb = '';
                                if($lubricant->image) {
                                    $thumb = str_starts_with($lubricant->image, 'http') ? $lubricant->image : asset('assets/images/lubricants/'.$lubricant->image);
                                }
                            @endphp
                            <img id="imagePreview" src="{{ $thumb }}" style="{{ $lubricant->image ? 'display:block' : 'display:none' }};max-width:100%;max-height:200px;border-radius:var(--radius-sm);margin:0 auto" alt="Preview">
                        </div>
    ...
    <div>
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <select name="status" class="form-select">
                        <option value="1" {{ old('status', $lubricant->status) == '1' ? 'selected' : '' }}>Active (Published)</option>
                        <option value="0" {{ old('status', $lubricant->status) == '0' ? 'selected' : '' }}>Inactive (Draft)</option>
                    </select>
                </div>
                <div class="form-group" style="margin-bottom:0">
                    <label class="form-check">
                        <input type="checkbox" name="featured" value="1" {{ old('featured', $lubricant->featured) ? 'checked' : '' }}>
        ...
        <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:13px" form="lubricantForm">
            <i class="fas fa-save"></i> Update Lubricant
        </button>
