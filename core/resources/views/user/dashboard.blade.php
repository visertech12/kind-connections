            ...
            <div class="alert-item">
                <div class="alert flex-align alert--warning" role="alert">
                    ...
                    <div class="alert__content">
                        ...
                        <p class="alert__desc">Renew support as soon as possible for peace of mind and to get support from us! <a href="{{ route('user.direct.purchases') }}" class="alert__link text--base fw-semibold">View Purchases</a> </p>
                    </div>
                </div>
            </div>
        @endif
                    ...
                    <tbody>
                        ...
                        @foreach ($tickets as $ticket)
                            @if ($ticket->status != 'Closed')
                                <tr>
                                    <td data-label="Department"><span class="fw-bold">{{ $ticket->deptname }}</span></td>
                                    <td data-label="Ticket">#{{ $ticket->tid }}</td>
                                    <td data-label="Subject">{{ shortDescription($ticket->subject, 50) }}</td>
                                    <td data-label="Status"><span class="{{ getBadge($ticket->status) }}">{{ $ticket->status }}</span></td>
                                    <td data-label="Last Response">{{ diffForHumans($ticket->lastreply) }}</td>
                                    <td data-label="Details">
                                        <a href="{{ route('user.ticket.details', $ticket->tid) }}" class="btn btn--base d-inline-block">Details</a>
                                    </td>
                                </tr>
                            @endif
                        @endforeach
                ...
                <div class="custom--card card">
                    ...
                    <div class="card-body">
                        @if ($latestPurchase->count())
                            <ul class="card-history">
                                @foreach ($latestPurchase as $purchase)
                                    @php
                                    ...
                                    <li class="card-history__item flex-between">
                                        <div class="card-history__inner flex-center">
                                            <span class="card-history__icon bg--base-two">
                                                <img src="{{ asset('assets/images/products/' . @$product->icon) }}" alt="" loading="lazy">
                                            </span>
                                            <div class="card-history__content">
                                                <h6 class="card-history__title fs-16">{{ $productName }}</h6>
                                                <p class="card-history__desc fs-14">#{{ $purchase->invid }}</p>
                                            </div>
                                        </div>
                                        <div class="card-history__right">
                                            ...
                                            @else
                                                ...
                                                <span class="card-history__right-time fs-14">{{ showDateTime($supportedUntil, 'Y-m-d') }}</span>
                                            @endif
                                        </div>
                                    </li>
                                @endforeach
