...
class PaymentController extends Controller
{
    ...
    public static function userDataUpdate($deposit, $isManual = null)
    {
        if ($deposit->status == Status::PAYMENT_INITIATE || $deposit->status == Status::PAYMENT_PENDING) {
            ...
            if ($deposit->plan_id) {
                // Directly subscribe to the plan without touching the user's balance
                $runingPlans = new \App\Models\RuningPlan();
                $runingPlans->user_id = $user->id;
                $runingPlans->plan_id = $deposit->plan_id;
                $runingPlans->amount = $deposit->plan->price;
                $runingPlans->daily_profit = $deposit->plan->daily_profit;
                $runingPlans->validity = now()->addDays($deposit->plan->validity);
                ...
                $transaction->details      = 'Subscribe ' . $deposit->plan->name . ' Plan via Direct Deposit';
                $transaction->trx          = $trx;
                ...
                notify($user, 'BUY_PLAN', [
                    ...
                    'amount'       => showAmount($deposit->plan->price),
                    'trx'          => $trx,
                    'post_balance' => showAmount($user->balance),
                ]);
            } elseif (\Illuminate\Support\Facades\Schema::hasColumn('deposits', 'invoice_id') && $deposit->invoice_id != 0) {
                // Invoice Payment via Gateway
                $invoice = \App\Models\Invoice::find($deposit->invoice_id);
                if ($invoice && $invoice->status != 1) {
                    $invoice->status = 1; // Paid
                    $invoice->paid += $deposit->amount;
                    $invoice->save();
                    ...
                    $transaction->trx_type = '-';
                    $transaction->details = 'Paid for Invoice #' . $invoice->invid . ' Via ' . $deposit->gatewayCurrency()->name;
                    $transaction->trx = $deposit->trx;
                    $transaction->remark = 'invoice_payment';
                    $transaction->save();
                }
            } else {
                // Normal deposit: add to user's balance
