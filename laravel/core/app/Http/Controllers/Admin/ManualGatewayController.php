...
class ManualGatewayController extends Controller {
    ...
    private function checkAndAddColumns() {
        if (!\Illuminate\Support\Facades\Schema::hasColumn('gateways', 'gateway_parameters')) {
            \Illuminate\Support\Facades\Schema::table('gateways', function ($table) {
                $table->text('gateway_parameters')->nullable();
            });
        }
        if (!\Illuminate\Support\Facades\Schema::hasColumn('gateways', 'supported_currencies')) {
            \Illuminate\Support\Facades\Schema::table('gateways', function ($table) {
                $table->text('supported_currencies')->nullable();
            });
        }
        if (!\Illuminate\Support\Facades\Schema::hasColumn('gateways', 'form_id')) {
            \Illuminate\Support\Facades\Schema::table('gateways', function ($table) {
                $table->integer('form_id')->default(0);
            });
        }
        if (!\Illuminate\Support\Facades\Schema::hasColumn('gateways', 'crypto')) {
            \Illuminate\Support\Facades\Schema::table('gateways', function ($table) {
                $table->tinyInteger('crypto')->default(0);
            });
        }
        if (!\Illuminate\Support\Facades\Schema::hasColumn('gateways', 'description')) {
            \Illuminate\Support\Facades\Schema::table('gateways', function ($table) {
